...
var formDataSaved = false;
	$(document).ready(function(event){
		$("#del_doc_img_btn").on('click',function(e){
			var _this = $(this);
			var docid = _this.data('docid');
			var imgname = _this.data('imgname');
			//console.log("docid:",docid);
			//console.log("prev:",_this.prev());
			//console.log('next:',_this.next().css({"display":"block"}));
			$('#loading_bar').css({"display":"inline"});
			$.ajax({
				async: true,
				type:'POST',
				dataType:'json',
				url:xb_global_namespace.baseurl+"admin_requests/doctor/remove_doc_image",
				data:{
				...
				success:function(datareceived,textStatus,jqXHR)
				{
					...
					{
						_this.parent().find('#image_doctor').val('');
						_this.prev().hide();
		...
		$(document).on('click','.del_clinic_img_btn',function(e){
			var _this = $(this);
			...
			$.ajax({
				...
				type:'POST',
				dataType:'json',
				url:xb_global_namespace.baseurl+"admin_requests/doctor/remove_clinic_image",
				data:{
						"image_id":imgid
					},
				success:function(datareceived,textStatus,jqXHR)
		...
		$('#update_doctor_form').on('submit',function(event){
			...
			{*/
				...
				tempObj = {"name":"docid","value":docid};
				...
				$.ajax({
					...
					type:'POST',
					dataType:'json',
					url:xb_global_namespace.baseurl+"admin_requests/doctor/updateDoctor",
					data:postData,
					success:function(datareceived,textStatus,jqXHR)
					{
						console.log(datareceived);
...
function initAutocomplete() {
				...
				var oldlat = parseFloat(document.getElementById('lat').dataset.lat);
				var oldlng = parseFloat(document.getElementById('lng').dataset.lng);

				//console.log({lat:oldlat,lng:oldlng});
