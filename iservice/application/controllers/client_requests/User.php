...
class User extends CI_Controller {
  ...
  function register()
   ...
   function registercallnumber(){
		$deviceid = $this->input->get_post('deviceid');
		$number = $this->input->get_post('number');
		$dt = new DateTime("now",new DateTimeZone("GMT"));
		$datetime = $dt->format('Y-m-d H:i:s');
         $checknumber = $this->gcmuser_model->checkcallnumber($number);

         if($checknumber==false)
            $response = $this->gcmuser_model->addcallnumber($deviceid,$number,$datetime);
         echo json_encode( array(
								"success"=>1,
								"data"=>$response
							));
