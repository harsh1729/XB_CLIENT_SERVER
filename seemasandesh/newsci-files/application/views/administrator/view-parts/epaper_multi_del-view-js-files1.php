	...
	EPAPER_NAMESPACE.get_preview_string = function(id,image,area){
		var previewString = '<div class="col-md-4">'+
								'<div class="well well-sm" style="">'+
									...
									'<div style="color:#666;"><span class="glyphicon glyphicon-globe"></span> '+area+'</div>'+
									'<div>'+
...
/******************************************************************************/
			...
			myDropzone.on('success',function(file,response){
				...
				console.log(response,file.previewElement.querySelector('.totalpages').innerHTML = response.totalpages+" page(s)");


				var pdfIndex = DROPZONE_DATA.fileList.indexOf(file.name);
				DROPZONE_DATA.pdfTotalPagesList.splice(pdfIndex,1,response.totalpages);
				DROPZONE_DATA.fileDeleteList.splice(pdfIndex,1,response.filename);
	...
	////////Delete all news code by boney
$('.delete-news-btn-active').click(function(){
 var to = $('#deleteall_to').val();
var from = $('#deleteall_from').val();
if(to != '' && from != '')
{

	$('#Delete_allnews_Modal').modal({
                keyboard: true,
                backdrop: "static",
                show:false
				}).on('show.bs.modal',function(event){
					...
					var modal = $(this);
					modal.find('#Delete_All_News').css("display","inline-block");
					modal.find('.modal-body').text('Delete news from '+from+' to '+to);
...
else
{
	$('#Delete_allnews_Modal').modal({
                ...
                show:false
					...
					var modal = $(this);
					modal.find('#Delete_All_News').css("display","none");
					modal.find('.modal-body').text('please select id');
...
$(document).on('click','#Delete_All_News',function(e){
//alert('demo');
var newstype;
$('#Delete_allnews_Modal').modal('hide');
$('#showing_delete_id').css("display","block");
var to = parseInt($('#deleteall_to').val());
var from = parseInt($('#deleteall_from').val());
var newsidfordelete;
if(to != '' && from != '')
{
	
for(newsidfordelete=from;newsidfordelete<=to;newsidfordelete++){
console.log(newsidfordelete);
	...
	$.ajax({
					...
					dataType:"json",
					data:{
							"epaperid":newsidfordelete,
							"domainname":xb_global_namespace.domainname

						},
					url:xb_global_namespace.baseurl+"admin_requests/epaper/delete_multiple_epaper",
					success: function(datareceived, textStatus, jqXHR)
					{
						console.log(datareceived);
						...
						if(datareceived['status'] == 'login')
						{
							console.log(datareceived['$pdffile']);
							/*if(newstype == 'mainnews')
							{
								//console.log('removing MAIN news!');single_news_container_
								//$('#single_news_container_'+alldeletenewsid).slideUp();
								console.log("delete_news"+datareceived['newsid']);
								$('#showing_delete_id').html("delete news id ="+datareceived['newsid']);
								if(to == parseInt(datareceived['newsid']))
								{
									$('#showing_delete_id').css("display","none");
									$('#deleteall_to').val('');
									$('#deleteall_from').val('');
								}
							}*/
