...
<script>
	...
	var copy_btn ="<input type='button'  data-id='newsheading' data-target='#copyModal' data-toggle='modal' class='unicode_convert_button btn btn-info btn-sm pull-right' value='copy' style='margin-bottom:20px;'>";
		PENDING_VIEW_NAMESPACE.generate_dom_string = function(newsid,heading,paragraph,image,place,reporter,daytime,imgtagline){
		...
		var domString =	'<div class="well">'+
							...
							'<div class="row">'+
								...
								'<div class="col-md-8">'+
									...
									'<div style="color:#666;cursor:pointer;" class="pull-right btn btn-xs aprove_news_btn" data-newsid="'+newsid+'" ><span class="glyphicon glyphicon-thumbs-up"></span> Aprove</div>'+	
								'</div>'+
							'</div>'+
							'<div class="row">'+
							 '<div class="col-md-12">'+
	...
	PENDING_VIEW_NAMESPACE.generate_edit_dom_string = function(id,heading,content,image,imgtagline){
		...
		var domString = '<hr style="border-width:1px;border-style:inset;display:none;" class="elements-added"/>'+
						'<div class="row elements-added" style="display:none;">'+
							'<div class="col-md-12">'+
								'<div contenteditable="true" class="lang_class form-control main-news-heading" style="color:#555;margin-bottom:;overflow:auto;" name="heading_'+id+'" id="heading_'+id+'">'+heading+'</div>'+copy_btn+'<br><input type="hidden" name="heading_'+id+'" >'+
								
								'<div class="row">'+
									'<div class="col-md-4">'+
										'<img style="height:220px;margin-bottom:5px;" src="'+image+'" class="img-responsive center-block main-news-image" />'+
										'<div contenteditable="true" class="lang_class form-control main-news-imgtagline" style="color:#666;overflow:auto;"  id="imgtagline_'+id+'">'+imgtagline+'</div>'+copy_btn+'<br><input type="hidden" name="imgtagline_'+id+'">'+
									'</div>'+
									'<div class="col-md-8">'+
										'<div style="color:#555;max-height:240px;height:240px;margin-bottom:;overflow:auto;" id="content_'+id+'" class="lang_class form-control main-news-content" contenteditable="true">'+content+'</div>'+copy_btn+'<br>'+'<br><input type="hidden" name="content_'+id+'" >'+
	...
	PENDING_VIEW_NAMESPACE.load_news = function(catid,fromdate,todate,calltype){
										//console.log(catid,fromdate,todate,calltype,PENDING_VIEW_NAMESPACE.lastnewsid);
										$.ajax({
											async:true,
											type:'POST',
											dataType:'json',
											...
											data:{
													...
													"enddate":todate,
													"calltype":calltype,
													"limit":PENDING_VIEW_NAMESPACE.limit,
													"offset":PENDING_VIEW_NAMESPACE.offset,
													"lastnewsid":PENDING_VIEW_NAMESPACE.lastnewsid,
											...
											success:function(datarecieved,textStatus,jqXHR)
											{
												...
												var usertype =datarecieved['usertype'];
												if(datarecieved['status'] == 'login')
												{
													if(data.length)
													{
														if(calltype == 'pending')
														{
															PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollEnable = true;
															PENDING_VIEW_NAMESPACE.offset = 0;
														}

														if(data[data.length - 1]['id'] > 0){
															PENDING_VIEW_NAMESPACE.lastnewsid = data[data.length - 1]['id'];
															console.log('lastnewsid is '+PENDING_VIEW_NAMESPACE.lastnewsid);
														}else{
															console.log('id not found');
														}
														PENDING_VIEW_NAMESPACE.offset = PENDING_VIEW_NAMESPACE.offset + data.length;
														$.each(data,function(index, el) {
															$(PENDING_VIEW_NAMESPACE.generate_dom_string(el.id,el.heading,el.content,el.image,'default place',el.reportername,el.datetime,el.imgtagline,usertype)).hide().appendTo('#pendingnewsContainerColumn').fadeIn(1000);
														});
													}
													else
													{
														PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollEnable = false;
													}
												}
												...
												PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollBusy = false;
	...
	PENDING_VIEW_NAMESPACE.lastnewsid = 0;
	PENDING_VIEW_NAMESPACE.offset = 0;
	PENDING_VIEW_NAMESPACE.limit = 2;
	PENDING_VIEW_NAMESPACE.myinfinitescroll = {};
	PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollEnable = false;	
	PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollBusy = false;	// is there any ongoing ajax request
	PENDING_VIEW_NAMESPACE.myinfinitescroll.loadBefore = 100; // load this much pixel before the scrollbar reaches to end!

	
	$(document).ready(function(){ 
		
		PENDING_VIEW_NAMESPACE.load_news($('#categoryId').val(),$('#fromDate').val(),$('#toDate').val(),"pending");
		...
		//$(PENDING_VIEW_NAMESPACE.generate_dom_string(1,"This is heading",'This is para','http://xercesblue.in/newsentry/uploads/14219140479887_r-day5.jpg','Sri Ganganagar','Vikas Sharma','02:30:23 23-02-2015')).appendTo('#pendingnewsContainerColumn');
		
		$('#categoryId').on('change',function(e){
			$('#pendingnewsContainerColumn').empty();
			PENDING_VIEW_NAMESPACE.lastnewsid = 0;
			PENDING_VIEW_NAMESPACE.offset = 0;
			PENDING_VIEW_NAMESPACE.load_news($(this).val(),$('#fromDate').val(),$('#toDate').val(),"pending");
		});
		...
		{
			if(PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollEnable && !PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollBusy && $(document).height() - $(window).height() - PENDING_VIEW_NAMESPACE.myinfinitescroll.loadBefore <= $(window).scrollTop() )
			{
				$('#infiniteloadingindicator').css('display','block');
				PENDING_VIEW_NAMESPACE.myinfinitescroll.scrollBusy = true;
				PENDING_VIEW_NAMESPACE.load_news($('#categoryId').val(),$('#fromDate').val(),$('#toDate').val(),"pending");
			}
		});
		...
		$('#customModal').modal({
                ...
                $(document).on('click','div.aprove_news_btn',function(event){
			var _this = $(this);
		  ...
		  var newsid = _this.data('newsid');
			...
			$.ajax({
				async:true,
				url:xb_global_namespace.baseurl+"admin_requests/news/approve_pending_news",
				dataType:'json',
				type:'POST',
				data:{
						"newsid":newsid,
			...
			$.ajax({
				...
				success:function(datareceived,textStatus,jqXHR)
				{
					...
					if(datareceived['status'] == 'login')
					{
						...
						_this.parent().parent().parent().replaceWith(PENDING_VIEW_NAMESPACE.generate_dom_string(el.id,el.heading,el.content,el.image,'default place',el.reportername,el.datetime,el.imgtagline));
				...
				success:function(datarecieved,textStatus,jqXHR)
				{
					...
					if(datarecieved['status'] == 'login')
					{
						if(data)
						{
							...
							_this.parent().parent().prepend('<div contenteditable="true" class="form-control lang_class" style="color:#555;overflow:auto;"  id="heading_'+data.id+'">'+data.heading+'</div>'+copy_btn+'<br><input type="hidden" name="heading_'+data.id+'">');
...
*/
							_this.parent().parent().find('.main-news-imgtagline').replaceWith('<div contenteditable="true" class="form-control main-news-imgtagline lang_class" style="color:#666;overflow:auto;" id="imgtagline_'+data.id+'" >'+data.imgtagline+'</div>'+copy_btn+'<br><input type="hidden" name="imgtagline_'+data.id+'" >');


							_this.parent().find('.main-news-content').replaceWith('<div contenteditable="true" style="color:#555;max-height:220px;height:220px;overflow:auto;margin-bottom:;" id="content_'+data.id+'" class="form-control main-news-content lang_class">'+data.content+'</div>'+copy_btn+'<br><input type="hidden" name="content_'+data.id+'">');
							...
							$.each(data.subnews,function(index,el){
								_this.parent().parent().parent().append(PENDING_VIEW_NAMESPACE.generate_edit_dom_string(el.id,el.heading,el.content,el.image,el.imgtagline));
							});
				...
				success:function(datarecieved,textStatus,jqXHR)
				{
					...
					if(datarecieved['status'] == 'login')
					{
						if(data)
						{
							...
							_this.parent().parent().parent().find('.main-news-heading').text(data.heading);
							_this.parent().parent().find('.main-news-image').attr('src',data.image);
							_this.parent().parent().find('.main-news-imgtagline').text(data.imgtagline);
							_this.parent().find('.main-news-content').css({
								'overflow-y':'scroll',
							...
							_this.parent().parent().parent().css({
								...
								'border-color':'#000',
										...
										1000, function() {});*/
							$.each(data.subnews,function(index,el){
								_this.parent().parent().parent().append(PENDING_VIEW_NAMESPACE.generate_view_dom_string(el.id,el.heading,el.content,el.image,el.imgtagline));
