...
<script>
	...
	NEWS_VIEW_NAMESPACE.generate_dom_string = function(heading,datetime,newsid,content,images,index){
		...
		{
			while(heading[heading_sub_index]!== " ")
			{
							 	heading_sub_index++;
		...
		$.ajax({                                                            
			...
			success:function(datareceived,textStatus,jqXHR)
			{
				...
				if(datareceived.length)
				{
					...
					{
						 //console.log(val);
						  //$('#contentcontainer').append(NEWS_VIEW_NAMESPACE.generate_dom_string(val.heading,val.datetime,val.newsid,val.content,val.images));
