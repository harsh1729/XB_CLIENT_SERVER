...
class News extends CI_Controller
{
	...
	function mob_get_cat_news()
	{
		...
		$catversion = $this->input->get_post('catversion');
		$appconfigversion = $this->input->get_post('appconfigversion');
		...
		*/
		if($clientid)
		{
			if($this->client_model->does_exists($clientid))
			{
				...
				$newsArray = $this->news_model->get_news_list($catid,$folders,'',$enddate,$limit,0,'fresh',$userid,$omit_news_id,$clientid,$domainname);
...
*/
		...
		if($clientid)
		{
			...
			$newsArray =  $this->news_model->get_news_list($catid,$folders,$startdate,$enddate,$limit,$lastnewsid,$calltype,$userid,$omit_news_id,$clientid,$domainname);
...
*/
		...
		if($clientid)
		{
			...
			foreach ($this->news_model->get_news_list($catid,$folders,$startdate,$enddate,$limit,$lastnewsid,$calltype,$userid,$omit_news_id) as $row)
			{
				$singlenews = array();
