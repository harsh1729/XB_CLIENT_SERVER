...
class Rashifal extends MY_Controller
{
	...
	function index()
	{
		if( $this->addons_model->client_have_addon($this->session->userdata('client_id'),"Rashifal",1) )
		{
			$dataToNav['page'] = "rashifal";
			...
			$navigationData['css'] = $this->load->view('administrator/view-parts/rashifal-view-css-files','',TRUE);
			$headerData['header'] = $this->load->view('administrator/view-parts/header-view',$navigationData,TRUE);
			
			$data['data'] = $this->load->view('administrator/rashifal-view',$headerData);
			
			$data['js'] = $this->load->view('administrator/view-parts/rashifal-view-js-files','',TRUE);
