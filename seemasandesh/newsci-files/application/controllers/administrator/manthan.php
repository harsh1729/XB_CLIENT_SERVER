<?php
class Manthan extends MY_Controller
{
	...
	function index()
	{
		if( $this->addons_model->client_have_addon($this->session->userdata('client_id'),"Manthan",1) )
		{
			$dataToNav['page'] = "manthan";
			...
			$navigationData['css'] = $this->load->view('administrator/view-parts/manthan-view-css-files','',TRUE);
			$headerData['header'] = $this->load->view('administrator/view-parts/header-view',$navigationData,TRUE);
			
			
			$data['data'] = $this->load->view('administrator/manthan-view',$headerData);
			
			$data['js'] = $this->load->view('administrator/view-parts/manthan-view-js-files','',TRUE);
