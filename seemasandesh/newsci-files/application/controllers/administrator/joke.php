...
class Joke extends MY_Controller
{
	...
	function index()
	{
		if( $this->addons_model->client_have_addon($this->session->userdata('client_id'),"Joke",1) )
		{
			...
			$navigationData['css'] = $this->load->view('administrator/view-parts/joke-view-css-files','',TRUE);
			...
			//$headerData['areas'] = $this->area_model->get_areas($folders,$clientid);
			...
			$data['data'] = $this->load->view('administrator/joke-view',$headerData);
			
			$data['js'] = $this->load->view('administrator/view-parts/joke-view-js-files','',TRUE);
